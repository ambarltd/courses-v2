...
class ProjectionReactionController extends AbstractController
{
    ...
    public function jsonPostRequestJsonResponse(
        ...
        try {
            ...
            $errorMessage = substr($exception->getMessage(), 0, 16_384);
            $stackTrace = substr($exception->getTraceAsString(), 0, 16_384);
            ...
            $errorMessage = substr($throwable->getMessage(), 0, 16_384);
            $stackTrace = substr($throwable->getTraceAsString(), 0, 16_384);
            $this->phpOutLogger->warning(\sprintf(
    ...
    private function requestJsonToRequestArray(Request $request): array
    {
        ...
        if (
            ...
            // && \array_key_exists('data_destination_description', $requestArray)
            && \array_key_exists('payload', $requestArray)
            && null !== $requestArray['data_source_id']
            // && null !== $requestArray['data_source_description']
            && null !== $requestArray['data_destination_id']
            // && null !== $requestArray['data_destination_description']
            && null !== $requestArray['payload']
        ) {
            return $requestArray;
        }
